    ...
    if (id) {
      await query(
        `UPDATE sms_templates 
         SET template_name = ?, sender_id = ?, sender_number = ?, matching_keyword = ?, is_active = ? 
         WHERE id = ? AND is_official = 1`,
        [template_name, sender_id, sender_number || null, matching_keyword || '', is_active === undefined ? 1 : (is_active ? 1 : 0), id]
      );
    } else {
      await query(
        `INSERT INTO sms_templates (template_name, sender_id, sender_number, matching_keyword, is_official, is_active) 
         VALUES (?, ?, ?, ?, 1, ?)`,
        [template_name, sender_id, sender_number || null, matching_keyword || '', is_active === undefined ? 1 : (is_active ? 1 : 0)]
      );
    }
...
async function getEmailAccounts(req, res) {
  try {
    const accounts = await query('SELECT * FROM smtp_gateways');
...
async function saveEmailAccount(req, res) {
  try {
    ...
    if (id) {
      ...
      await query(
        `INSERT INTO smtp_gateways (email, password, host, port, secure, daily_limit, is_active) 
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
...
async function deleteEmailAccount(req, res) {
  try {
    const { id } = req.params;
    await query('DELETE FROM smtp_gateways WHERE id = ?', [id]);
...
async function getSmsSettings(req, res) {
  try {
    const settings = await query('SELECT * FROM sms_gateways');
...
async function saveSmsSettings(req, res) {
  try {
    ...
    if (id) {
      ...
      await query(
        `INSERT INTO sms_gateways (gateway_url, http_method, post_body_template, api_key, username, sender_id, is_active, provider_name) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    ...
    if (activeFlag === 1) {
      ...
      const [savedRow] = await query(
        'SELECT id FROM sms_gateways WHERE gateway_url = ? ORDER BY id DESC LIMIT 1',
        [gateway_url]
      );
      if (savedRow) {
        await query(
          'UPDATE sms_gateways SET is_active = 0 WHERE id != ?',
          [savedRow.id]
        );
...
async function deleteSmsSettings(req, res) {
  try {
    ...
    await query('DELETE FROM sms_gateways WHERE id = ?', [id]);
...
async function resetEmailCounter(req, res) {
  try {
    ...
    await query('UPDATE smtp_gateways SET sent_today = 0 WHERE id = ?', [id]);
...
async function resetAllEmailCounters(req, res) {
  try {
    await query('UPDATE smtp_gateways SET sent_today = 0');
...
async function updateOtpFormat(req, res) {
  try {
    ...
    await query(
      "INSERT INTO otp_sms_templates (setting_key, setting_value) VALUES ('otp_format_template', ?) ON DUPLICATE KEY UPDATE setting_value = ?",
      [template, template]
    );

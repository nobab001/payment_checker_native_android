...
async function getTemplates(req, res) {
  try {
    const rows = await query(
      `SELECT id, template_name, sender_id, sender_number, matching_keyword, '' AS regex_pattern, is_official, is_active
         FROM sms_templates
        WHERE is_active = 1`
    );
...
async function addGatewayMethod(req, res) {
  try {
    ...
    const result = await query(
      `INSERT INTO gateway_methods (user_id, template_id, sim_slot, provider, number, is_enabled, priority)
       VALUES (?, ?, ?, ?, ?, 1, ?)`,

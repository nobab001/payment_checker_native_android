...
async function verifyCredential(req, res) {
  try {
    ...
    const users = await query('SELECT phone, email FROM users WHERE id = ?', [userId]);
    let primaryUpdated = false;
    if (users.length > 0) {
      if (type === 'email' && !users[0].email) {
        await query('UPDATE users SET email = ? WHERE id = ?', [cleanContact, userId]);
        primaryUpdated = true;
      } else if (type === 'phone' && !users[0].phone) {
        await query('UPDATE users SET phone = ? WHERE id = ?', [cleanContact, userId]);
        primaryUpdated = true;
      }
    }
    ...
    const exists = await query(
      "SELECT id FROM user_credentials WHERE user_id = ? AND value = ?",
      [userId, cleanContact]
    );

    if (exists.length > 0) {
      await query(
        "UPDATE user_credentials SET verified_at = NOW() WHERE id = ?",
        [exists[0].id]
      );
    } else {
      await query(
        "INSERT INTO user_credentials (user_id, type, value, verified_at) VALUES (?, ?, ?, NOW())",
        [userId, type, cleanContact]
      );
    }

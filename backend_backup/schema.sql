...
CREATE TABLE IF NOT EXISTS `sms_templates` (
  ...
  `sender_number`    VARCHAR(64)  DEFAULT NULL COMMENT 'Sender number/address for 4-step verification e.g. bKash or 01XXXXXXXXX',
...
-- Default templates
INSERT INTO `sms_templates` (`id`, `template_name`, `sender_id`, `sender_number`, `matching_keyword`, `is_official`, `is_active`) VALUES
  (1, 'bKash Personal', 'bKash', 'bKash', 'Cash In Tk,You have received Tk', 1, 1),
  (2, 'Nagad Personal', 'NAGAD', 'NAGAD', 'received cash in Tk,TrxID:', 1, 1),
  (3, 'Rocket Personal', '16216', '16216', 'received Tk,TrxID:', 1, 1),
  (4, 'Upay Personal', 'upay', 'upay', 'received Tk,TrxID', 1, 1),
  (5, 'bKash Agent', 'bKash', 'bKash', 'Cash In,Tk.,Ref:', 1, 1),
  (6, 'Nagad Agent', 'NAGAD', 'NAGAD', 'Cash in received,Tk.,TrxID:', 1, 1),
  (7, 'Rocket Agent', '16216', '16216', 'Cash In received,Tk.,TrxID:', 1, 1),
  (8, 'Upay Agent', 'upay', 'upay', 'Cash In received,Tk.,TrxID', 1, 1)
ON DUPLICATE KEY UPDATE 
...
--   9. sms_gateways        — SMS gateway provider config
--   10. smtp_gateways     — SMTP email configuration details
...
-- sms_templates table: sender_number for 4-step SMS verification
ALTER TABLE `sms_templates`
  ADD COLUMN IF NOT EXISTS `sender_number` VARCHAR(64) DEFAULT NULL COMMENT 'Sender number/address for 4-step verification e.g. bKash or 01XXXXXXXXX' AFTER `sender_id`;

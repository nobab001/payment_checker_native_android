...
router.post('/pin/reset-send-otp',  otpRateLimiter, pin.resetPinSendOtp);

// POST /api/pin/reset-verify     → OTP যাচাই করে নতুন PIN সেট করো (no auth)
router.post('/pin/reset-verify',    otpRateLimiter, pin.resetPinVerify);

// POST /api/pin/verify           → PIN যাচাই করো (auth required)
router.post('/pin/verify',          auth, otpRateLimiter, pin.verifyPin);

...
object AppConfig {
    ...
    const val KEY_GLOBAL_BLOCKED_SENDERS = "pcu_global_blocked_senders"
    ...
    const val KEY_PERM_MANUAL_TXN       = "pcu_perm_manual_transaction"
    const val KEY_EFF_MAX_DEVICES       = "pcu_eff_max_devices"
    const val KEY_EFF_MAX_SITES         = "pcu_eff_max_sites"
    const val KEY_SUBSCRIPTION_STATUS   = "pcu_subscription_status"
    const val KEY_CUSTOM_ALL_CHIP_LABEL = "pcu_custom_all_chip_label"
    const val KEY_CUSTOM_ALL_POPUP_TITLE = "pcu_custom_all_popup_title"
    ...
    const val HEARTBEAT_INTERVAL_MS         = 900_000L     // 15 min (welcome/gateway)
    const val HEARTBEAT_PERSONAL_BUSINESS_MS = 1_800_000L  // 30 min
    const val HEARTBEAT_SPARSE_MS           = 3_600_000L  // 60 min (personal)
